...
| **TestFlight / pre-release** | **3.20** (and later builds) | Companion **1.9.2** is supported: long-term parking sleep/wake timeline, observed plug/security/climate events, verified drive trajectories, and optional Live Activities / software push after secure pairing. |
...
**Recommended Companion version:** public release **1.9.2**. Older notes that mentioned Companion 1.5.0 referred only to the first software-notification server piece; current capabilities are documented with 1.9.2 in [COMPATIBILITY.md](COMPATIBILITY.md).
...
2. My T Companion **1.9.2** (or newer with the same capabilities).
...
| **TestFlight / 预发布** | **3.20**（及后续构建） | 支持 Companion **1.9.2**：长期停车休眠/唤醒流水、插枪/安防/空调等真实观测事件、可验证行驶轨迹，以及完成安全配对后的可选实时活动与软件推送。 |

3.10 已有的 TeslaMate、TeslaMateAPI 连接、行程、充电、统计等功能不依赖 My T Companion。

**推荐 Companion 版本：** 公开版 **1.9.2**。文档中若仍出现 1.5.0，仅指早期软件通知服务端能力；当前能力以 1.9.2 与 [COMPATIBILITY.md](COMPATIBILITY.md) 为准。

可选的车辆软件通知、充电实时活动、导航实时活动需要：

1. 兼容的 My T（TestFlight 3.20+；非 App Store 3.10）。
2. My T Companion **1.9.2**（或具备相同能力的更新版）。
...
| **TestFlight / 預發布** | **3.20**（及後續建置） | 支援 Companion **1.9.2**：長期停車休眠／喚醒流水、插槍／安防／空調等真實觀測事件、可驗證行駛軌跡，以及完成安全配對後的可選即時動態與軟體推播。 |

3.10 已有的 TeslaMate、TeslaMateAPI 連線、行程、充電、統計等功能不依賴 My T Companion。

**建議 Companion 版本：** 公開版 **1.9.2**。文件中若仍出現 1.5.0，僅指早期軟體通知伺服器端能力；目前能力以 1.9.2 與 [COMPATIBILITY.md](COMPATIBILITY.md) 為準。

可選的車輛軟體通知、充電即時動態、導航即時動態需要：

1. 相容的 My T（TestFlight 3.20+；非 App Store 3.10）。
2. My T Companion **1.9.2**（或具備相同能力的更新版）。
