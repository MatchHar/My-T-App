...
| --- | --- | --- |
| **Public App Store** | **My T 3.10** | Downloadable from Apple’s public listing. The listing, not an internal review note, is the source of truth for public availability. |
...
| **My T Companion** | [latest stable release](https://github.com/MatchHar/My-T-Companion/releases/latest) — verified as **1.10.19** | My T discovers capabilities at `/api/v1/capabilities`; compatibility does not depend on a hard-coded version alone. |
...
| 渠道 | 已核对版本 | 可用状态 |
| --- | --- | --- |
| **App Store 公开版** | **My T 3.10** | 可从 Apple 公开页面下载。公开可用状态以 App Store 页面为准，不以内部审核记录为准。 |
| **预发布开发版** | 更新版本 | 截图和文档可能介绍仍在为后续 App Store 版本验证的能力；Apple 页面更新前，不会把这些能力描述成已公开下载。 |
| **My T Companion** | [最新稳定版](https://github.com/MatchHar/My-T-Companion/releases/latest) — 已核对为 **1.10.19** | My T 通过 `/api/v1/capabilities` 发现能力；兼容性不只依赖写死的版本数字。 |
...
| 渠道 | 已核對版本 | 可用狀態 |
| --- | --- | --- |
| **App Store 公開版** | **My T 3.10** | 可從 Apple 公開頁面下載。公開可用狀態以 App Store 頁面為準，不以內部審查記錄為準。 |
| **預發佈開發版** | 更新版本 | 截圖及文件可能介紹仍在為後續 App Store 版本驗證的能力；Apple 頁面更新前，不會將這些能力描述為已公開下載。 |
| **My T Companion** | [最新穩定版](https://github.com/MatchHar/My-T-Companion/releases/latest) — 已核對為 **1.10.19** | My T 透過 `/api/v1/capabilities` 探測能力；相容性不只依賴寫死的版本數字。 |
