...
The server companion is optional. A future compatible My T build will continue
to work without it and will enable enhanced history only after detecting the
companion through the existing TeslaMateAPI address.
...
只有通过现有 TeslaMateAPI 地址检测到组件后，才会启用增强历史。
...
只有經現有 TeslaMateAPI 位址偵測到元件後，才會啟用增強歷史。
